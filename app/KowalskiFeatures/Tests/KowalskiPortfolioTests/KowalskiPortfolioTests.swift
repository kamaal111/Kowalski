...
struct KowalskiPortfolioTests {
    ...
    @Test
    func `Fetch dashboards should default cache currency to app fallback when active currency is unavailable`(
    ) async throws {
        let cacheDirectory = try makeDashboardCacheDirectory()
        let portfolioClient = MockPortfolioClient(dashboardsResult: .success(makePortfolioDashboardsResponse()))
        let portfolio = KowalskiPortfolio.testing(
            client: .testing(portfolio: portfolioClient),
            dashboardCacheDirectoryURL: cacheDirectory,
        )
        ...
        let cacheFileURL = PortfolioDashboardCache(directoryURL: cacheDirectory).cacheFileURL(
            sessionEmail: "",
            currencyCode: KowalskiFeatureDefaults.fallbackCurrency.rawValue,
            period: .oneYear,
        )
        #expect(FileManager.default.fileExists(atPath: cacheFileURL.path()))
        #expect(try dashboardCacheFileCount(in: cacheDirectory) == 1)
    ...
    @Test
    func `Overview fetch should use the server-backed net worth on repeated refreshes`() async throws {
        ...
        #expect(portfolio.netWorth?.value == 100)
        #expect(portfolio.allTimeProfit?.value == 0)
        #expect(portfolio.allTimeProfitPercentage == 0)
    }

    @Test
    func `Overview fetch should preserve server net worth when transaction cost basis currency is mixed`() async throws {
        let overview = makePortfolioOverviewResponse(
            transactions: [
                makePortfolioEntryResponse(
                    stock: makeAppleStockResponse(),
        ...
        #expect(portfolio.netWorth?.currency == .EUR)
        #expect(portfolio.netWorth?.value == 350)
...
private actor MockPortfolioClient: KowalskiPortfolioClient {
    ...
    func getDashboards(
        ...
        guard !dashboardsResults.isEmpty else { return .success(makePortfolioDashboardsResponse()) }
